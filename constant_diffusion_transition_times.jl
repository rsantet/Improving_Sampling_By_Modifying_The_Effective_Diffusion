...
effective_diffusion_squared = 1.0 # Set the constant diffusion

function constant_diffusion_compute_transition_times(scheme, Δt, n_transitions::Int, set_tolerance; n_loggers=nothing, γ=1.0)

    @assert isapprox(effective_diffusion_squared, 1.0)
    ...
    println("Constant diffusion\tRunning for Δt = ", Δt)

    ## Run the long trajectory
    # Initial condition
    initial_z = 0.0
    if scheme == "MALA"
        q, F, ∇z, unit_vector, distance, projection_matrix, D, sqrt_D, inv_D, div_D, κ_α = initialize_OL(initial_z, 0.)
    elseif scheme == "adaptive_MALA"
        q, F, ∇z, unit_vector, distance, projection_matrix, D, sqrt_D, inv_D, div_D, MeanForce, FreeEnergy, MeanForceUpdate, κ_α, sqrt_κ_α = initialize_adaptive_OL(initial_z, 0.)
    elseif scheme == "RMHMC" || scheme == "RMGHMC"
        q, p, F, ∇z, ∇2z, unit_vector, distance, projection_matrix, inv_sqrt_D, κ_α = initialize_HMC(initial_z, 0.)
    end
    ...
    while n_transitions_counter < n_transitions
        ...
        if !isnothing(n_loggers)
            if n_iterations_counter % n_loggers == 0
                ...
                println("$scheme, Constant diffusiont\t Δt = ", round(Δt, sigdigits=5), "\t Transition\t", n_transitions_counter, "\tIteration\t", n_iterations_counter, "\t RC:\t", z)
            end
        end

        # Update
        if scheme == "RMHMC"
            one_step_RMHMC!(q, p, F, ∇z, ∇2z, unit_vector, distance, projection_matrix, inv_sqrt_D, 0., κ_α, Δt, rejection_counter)
        elseif scheme == "RMGHMC"
            one_step_RMGHMC!(q, p, F, ∇z, ∇2z, unit_vector, distance, projection_matrix, 0., κ_α, Δt, γ, rejection_counter)
        elseif scheme == "MALA"
            one_step_MALA!(q, F, ∇z, unit_vector, distance, projection_matrix, D, sqrt_D, div_D, inv_D, Δt, 0., κ_α, rejection_counter)
        elseif scheme == "adaptive_MALA"

            if n_iterations % n_mean_force_update == 0
                update_mean_force!(MeanForceUpdate, MeanForce)
                update_free_energy!(MeanForce, FreeEnergy)
                κ_α, sqrt_κ_α = update_κ_α(FreeEnergy, 0.)
            end

            one_step_adaptive_MALA!(q, F, ∇z, unit_vector, distance, projection_matrix, D, sqrt_D, div_D, inv_D, Δt, 0., κ_α, sqrt_κ_α, MeanForce, MeanForce, FreeEnergy, rejection_counter)
        end
        ...
        if compact_form
            if is_in_expanded_form(q, unit_vector, distance, set_tolerance)
                ...
                println("$scheme, Constant diffusion \t Δt = ", round(Δt, sigdigits=5), "\t Transition 0 -> 1 has occurred after ", n_iterations_counter, " iterations. Remaining transitions: ", n_transitions - n_transitions_counter)
            ...
            if is_in_compact_form(q, unit_vector, distance, set_tolerance)
                ...
                println("$scheme, Constant diffusion \t Δt = ", round(Δt, sigdigits=5), "\t Transition 1 -> 0 has occurred after ", n_iterations_counter, " iterations. Remaining transitions: ", n_transitions - n_transitions_counter)
...
# res = constant_diffusion_compute_transition_times(scheme, Δt, n_transitions, set_tolerance; γ)
# using Statistics
# println(mean(res[2]))